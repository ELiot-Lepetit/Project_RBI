...
The Datasets table represents the 11 raw data sets before the cleaning and merging which we will use for visualization and interpretation of our research questions/interests. The number of rows depicts the the number of states and union territories and the number of columns represent the availability of data with respect to the indicators and Net State Domestic Product by year.
...
- Why these Datasets:- 
The relationship between socio-demographic indicators and economic performance analyze the complexities of regional development in a diverse country like India. This research seeks to investigate how socio-demographic factors, such as literacy rate, health expenditure, birth rate, and other indicators, interact with the Net State Domestic Product (NSDP) to shape the economic outcomes of Indian states. By linking these indicators, we aim to uncover patterns, disparities, and trends that explain variations in development outcomes across different states of India.

The NSDP data reflects the capacity of states to generate economic value, adjusted for inflation, allowing for meaningful comparisons over time. On the other hand, socio-demographic indicators reveal the underlying health and human capital conditions that can either drive or hinder economic growth. Together, these datasets provide a robust foundation for analysis of regional disparities, enabling us to examine how social conditions influence economic outcomes and vice versa.
...
```{r}
datasets <- list(
  Literacy_data, Birth_data, Sex_data, Death_data, Pop_data, Health_Exp_data, NSDP_clean, TFR_data, IMR_data, Literacy_data
)

# Names of the datasets
dataset_names <- c(
  "Literacy Rate", "Birth Rate", "Sex Ratio", "Death Rate", "Population", "Health Expenditure", 
  "NSDP", "Total Fertility Rate", "Infant Mortality Rate", "Literacy"
)
