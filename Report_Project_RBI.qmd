...
To address this research question, we propose two sets of hypothesis which aim to uncover key drivers of regional disparities and shed light on the connections between economic performance and social development across Indian states^[In the context of this analysis, the term "states" is used inclusively to refer to both Indian states and Union Territories, unless otherwise specified.].
...
knitr::kable(table_2019, caption = "Sample Dataset - First Five Observations for Hypothesis 1", align = "c")
...
knitr::kable(table_2011, caption = "Sample Dataset - First Five Observations for Hypothesis 2", align = "c")
```
...
3.	Clustering: Most states are clustered in the lower NSDP and Health Expenditure range, indicating limited fiscal capacity and health spending for the majority of states. These states may require targeted support to improve health expenditure despite their lower fiscal capacity.
...
2.  Population Effect: On the one hand, states with larger populations often face challenges in achieving high per capita health expenditure whereas, on the other hand, regions such as Nagaland and Meghalaya highlight how strategic allocation of health resources can achieve elevated per capita health expenditure, even with constrained fiscal capacity.
...
ggplot(merged_data_2019_clean, aes(x = `Health Expenditure Per Capita`, y = `Death Rate`, color = State)) +
  geom_point() +
    geom_text_repel(aes(label = State), size = 2.2) +  
  geom_smooth(method = "lm", formula = y ~ x, color = "red", se = FALSE) +
  labs(
...
# First Table: Health Expenditure & Infant Mortality Rate Analysis
correlation1 <- cor(merged_data_2019_clean$`Health Expenditure Per Capita`, merged_data_2019_clean$`Infant Mortality Rate`)

model1 <- lm(`Infant Mortality Rate` ~ `Health Expenditure Per Capita`, data = merged_data_2019_clean)
summary_model1 <- summary(model1)

results_table1 <- data.frame(
  Statistic = c("Correlation Coefficient", "Intercept", "Slope", "R-squared", "Adjusted R-squared", "p-value"),
  Value = c(
    round(correlation1, 3),
    round(summary_model1$coefficients[1, 1], 3),
    round(summary_model1$coefficients[2, 1], 6),
    round(summary_model1$r.squared, 3),
    round(summary_model1$adj.r.squared, 3),
    round(summary_model1$coefficients[2, 4], 3)
  )
)

# Second Table: Health Expenditure & Death Rate Analysis
correlation2 <- cor(merged_data_2019_clean$`Health Expenditure Per Capita`, merged_data_2019_clean$`Death Rate`)

model2 <- lm(`Death Rate` ~ `Health Expenditure Per Capita`, data = merged_data_2019_clean)
summary_model1 <- summary(model1)
...
kable(results_table1, caption = "Overview of Correlation and Linear Regression Findings for the 'Infant Mortality Rate Analysis")  %>%
  kable_styling(position = "center", full_width = FALSE) %>%  
  row_spec(0, align = "c")        


kable(results_table2, caption = "Overview of Correlation and Linear Regression Findings for the Health Expenditure and Death Rate Analysis"  
) %>%
  kable_styling(position = "center", full_width = FALSE) %>%
  row_spec(0, align = "c")
```
In these tables, we examine the correlation and linear regression results to explore the relationship between Health Expenditure with Infant Mortality Rate and Death Rate. 

#### Conclusion:
1.	While there is a slight negative relationship between health expenditure per capita and mortality metrics, the relationship is too weak and statistically insignificant to draw strong or even any conclusions.
2.	Other factors, such as healthcare quality, public health policies, demographic characteristics, or disease prevalence, likely have a much larger impact on mortality outcomes.

#### Limitations
1.  Weak Predictive Power and Statistical Insignificance: the very low R-squared values and high p-values show that health expenditure per capita has little to no impact on these outcomes. This suggests the need to investigate additional factors, such as healthcare access, literacy rates, or disease burden, that might better explain the variations in mortality.
2.  Scale of Analysis: the analysis is conducted at the state level, which overlooks regional and intra-state variations. Differences in healthcare infrastructure, population density, and resource allocation within states could obscure the true relationship
...
ggplot(Filter_data_2011, aes(x = `Literacy Rate`, y = `Sex Ratio`, color = State)) +
  ...
  geom_smooth(method = "lm", formula = y ~ x, color = "red", se = FALSE) +  # Add regression line
  labs( title = "Relationship Between Sex Ratio and Literacy Rate", y = " Sex Ratio (F per thousand M)", x = "Literacy Rate (%)", color = "State") +
...
Moreover, these results seem to be generalizable for all States in India, as demonstrated by the graph  and regression table below: 
```{r, fig.align="center"}
#| message: false
#| warning: false
#| echo: false
...
This report examined the interplay between socio-demographic indicators and Net State Domestic Product (NSDP) across Indian states, shedding light on regional disparities in economic and social development.

In Block 1, using 2019 data, we observed that states with higher NSDP allocate more resources to health expenditure. While higher health expenditure was linked to lower mortality rates, the relationship remained weak and statistically insignificant, highlighting the need for multivariate approaches. On a per-capita level, population effects became evident, with smaller states like Nagaland and Meghalaya achieving higher per-capita health expenditures despite limited fiscal resources, while larger states like Uttar Pradesh struggled to translate economic output into per-capita investments.

In Block 2, we use 2011 data and observe a positive correlation between NSDP and literacy rates, suggesting that states with stronger economies tend to achieve higher educational outcomes. However, the relationship lacks statistical significance, indicating that factors beyond economic output—such as policy focus, infrastructure, and cultural dynamics—play a substantial role. Furthermore, literacy rates were hypothesized to influence demographic outcomes. The analysis supports a strong negative correlation between literacy rates and birth rates, emphasizing the transformative impact of education on family planning and population control. Conversely, the relationship between literacy rates and sex ratios reveals unexpected results, with a weak and negative association that underscores the enduring influence of cultural and historical factors in shaping gender dynamics in India. We also refer some research to back our reasoning for the same. 

While our findings provide valuable insights, the limitations of small sample size, univariate models, and the use of 2011 and 2019 data constrain the generalizability of conclusions. Expanding the dataset and incorporating multivariate analyses could enhance the robustness of these observations.

Policy implications which can be derived from your analysis are:

1.  Targeted Investment in Health: Expand programs like Ayushman Bharat and improve primary healthcare infrastructure, focusing on regions with high mortality rates and low fiscal capacity.

2.  Strengthening Education Systems: Enhance initiatives like Samagra Shiksha Abhiyan to address regional disparities in literacy through investments in teacher training, infrastructure, and technology.

3. Promoting Gender Equity: Address cultural norms through programs like Beti Bachao Beti Padhao to improve the weak correlations between literacy and sex ratios.

4. Prioritize consistent data collection across socio-demographic and economic indicators to enable robust, evidence-based decision-making. Collaborate with organizations like the Reserve Bank of India (RBI) and National Sample Survey Office (NSSO) to ensure more reliable and comprehensive insights

In conclusion, this report highlights the intricate relationship between socio-demographic indicators and economic performance across Indian states in year 2011 and 2019, revealing both progress and disparities. 
Addressing these disparities requires collaborative efforts among policymakers, educators, healthcare providers, and regional governments to implement effective interventions. By prioritizing investments in health, education, and gender equity, India can foster inclusive development, unlock regional potential, and pave the way for sustainable and equitable growth.
...
```
# V) Acknowledgments
We extend our sincere gratitude to Mr. Fabrice Rossi, whose guidance and insights throughout the lectures were invaluable in shaping this project, and it provided a strong foundation for conducting this analysis and crafting meaningful conclusions. We also wish to thank the Reserve Bank of India (RBI) for providing access to the comprehensive datasets used in this study. The availability of such detailed and reliable data was instrumental in facilitating our research on socio-demographic and economic indicators across Indian states.^[The number of words in the report is `r word_count`. To count the words, we excluded all words that are part of the code (inside code chunks). Hence, the word count does not include words within tables, graphs, or any other elements generated through code.]
--- 
