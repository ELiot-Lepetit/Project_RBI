...
#| echo: false
library(ggrepel) 
ggplot(merged_data_2019_clean, aes(x = NSDP, y = `Health Expenditure`, color = State)) +
  geom_point(size = 1) +
  geom_text_repel(aes(label = State), size = 3) +  
  geom_smooth(method = "lm", formula = y ~ x, color = "red", se = FALSE) + 
  labs(
    title = "Relationship Between NSDP and Health Expenditure",
    x = "NSDP (Rupees in Crores)",
    y = "Health Expenditure (in Crores)"
  ) +
  theme_minimal() + 
...
kable(results_table, caption = "NSDP and Health Expenditure: Correlation and Regression Summary")  %>%
...
These limitations highlight the need for a per capita analysis, offering a more nuanced view of health expenditure relative to individual fiscal capacity and well-being.
...
ggplot(merged_data_2019_clean, aes(x = `NSDP Per Capita`, y = `Health Expenditure Per Capita`, size = Population / 1e6, color = State)) +
  ...
  labs(
    ...
    size = "Population (in millions)"  # Adjust size legend
  ) +
  scale_size_continuous(range = c(1, 8)) +  # Diminish size of points
  theme_minimal() +
  guides(color = "none") +  # Remove the color legend for states
  theme(
    legend.position = "right"  # Position the population legend to the right
  )
