...
text <- readLines("Project_RBI.qmd")
...
1.	Negative Correlation: The graph displays a negative slope, indicating an inverse relationship between Literacy Rate and Birth Rate. States with higher literacy rates, such as Maharashtra, tend to have lower birth rates, whereas states like Rajasthan, with lower literacy rates, exhibit higher birth rates. This aligns with the hypothesis that higher literacy rates are associated with lower birth rates due to increased awareness, education, and access to family planning.
...
#| warning: false
#| echo: false
# Compute correlation for Birth Rate and Literacy Rate
...
results_table_birth_rate <- data.frame(
  Statistic = c("Correlation Coefficient", "Intercept", "Slope (Literacy Rate)", "R-squared", "Adjusted R-squared", "p-value"),
  Value = c(
    round(correlation_birth_rate, 3),                             # Correlation coefficient
    round(summary_model_birth_rate$coefficients[1, 1], 3),        # Intercept
    round(summary_model_birth_rate$coefficients[2, 1], 6),        # Slope
    round(summary_model_birth_rate$r.squared, 3),                 # R-squared
    round(summary_model_birth_rate$adj.r.squared, 3),             # Adjusted R-squared
    round(summary_model_birth_rate$coefficients[2, 4], 3)         # p-value for the slope
  )
)
...
kable(results_table_birth_rate, caption = "Summary of Correlation and Linear Regression Results for Birth Rate and Literacy Rate")  %>%
  ...
  row_spec(0, align = "c")
```
In this Table, we analyze the correlation to determine the strength and direction of the linear relationship between Literacy Rate and Birth Rate. Linear regression provides deeper insights by modeling this relationship, attempting to explain how much the dependent variable (Birth Rate) changes with the independent variable (Literacy Rate).


#### Conclusion

1.	Correlation: Literacy Rate and Birth Rate exhibit a strong negative correlation (-0.889). This indicates that, in 2011, as the Literacy Rate increased, the Birth Rate (number of live births per 1,000 people annually) tended to decrease significantly.
2.	Regression: The slope for Literacy Rate is nonzero, negative, and significant. The slope (-0.6398) indicates that for a 1% increase in Literacy Rate, the Birth Rate decreases by 0.6398 on average. The p-value of 0.01776 for the regression slope is less than 0.05, indicating that the relationship between Literacy Rate and Birth Rate is statistically significant. The R-squared value (0.7905) shows that 79.05% of the variation in Birth Rate is explained by Literacy Rate, which is a strong explanatory power for the model.
3.	Significance of the Model: The low residual standard error (1.955) and the high F-statistic (15.09) indicate a robust model. The regression results suggest that Literacy Rate is a strong and significant predictor of Birth Rate.
	Inference

The analysis confirms the hypothesis that higher Literacy Rates are associated with lower Birth Rates. This suggests that education plays a crucial role in family planning, urbanization, and access to reproductive health services, which collectively lead to a decline in Birth Rates. The negative slope and statistically significant results support the idea that states with better educational outcomes tend to have lower fertility rates. Therefore, Education is an important tool for improving socio-economic outcomes and controlling population growth. However, expanding the analysis to include additional variables and a larger dataset would provide a more comprehensive understanding of the factors influencing the Birth Rate in India. ^[The no of words in the report are `r word_count`.]