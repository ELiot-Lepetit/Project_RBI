...
#| warning: false
#| echo: false
Filter_data_2019 <- Filter_data %>%
  filter(Year == 2019) 

# Select specific columns for the hypothesis
Filter_data_2019 <- Filter_data_2019 %>% select(State, Year, NSDP, `Health Expenditure`, `Death Rate`, `Infant Mortality Rate`)
...
Filter_data_2019 <- Filter_data_2019 |> 
  mutate(NSDP = as.numeric(NSDP), `Health Expenditure` = as.numeric(`Health Expenditure`), `Death Rate` = as.numeric(`Death Rate`),`Infant Mortality Rate` = as.numeric(`Infant Mortality Rate`) )
...
#| echo: false
# Display the table
kable(Filter_data_2019, caption = "Table: Selected States from Filtered Data (2019)")  %>%
...
kable(results_table_birth_rate2, caption = "Summary of Correlation and Linear Regression Results for Birth Rate and Literacy Rate")  %>%
  kable_styling(position = "center", full_width = FALSE) %>%  # Center the table
  row_spec(0, align = "c")
```
